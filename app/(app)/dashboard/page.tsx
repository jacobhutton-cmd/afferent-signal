...
export default async function DashboardPage() {
  ...
  const [{ count: areaCount }, { count: activeUsers }, { count: campaignCount }] = await Promise.all([
    supabase.from('users').select('*', { count: 'exact', head: true }).eq('zip_code', profile?.zip_code ?? ''),
    supabase.from('users').select('*', { count: 'exact', head: true }).eq('zip_code', profile?.zip_code ?? '').eq('premium_status', false),
    ...
    <AppShell>
      <div className="pt-4">
        ...
        <div className="flex gap-2 mb-6">
          <Link href="/submit" className="flex-1 py-2.5 text-center text-sm font-semibold rounded-xl text-white" style={{background: 'linear-gradient(to right, #2563EB, #16A34A)'}}>
            Submit Product
          </Link>
